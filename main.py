...
class condition:
    def __init__(
        ...
        GPA=None,
        grade=None,
        credit_upperbound=None,
        credit_lowerbound=None,
        require=None,
        general=None,
    ):
...
def interface(Course_number):
    ...
    if require[0] != "N":
        ...
        while position < len(require):
            while require[position] not in Course_number:
                print("目前科號:", require)
                temp = input(
                    "科號" + require[position] + "不存在，請問是否要修正此科號?\n如要修正，請重新輸入正確科號"
                    "如不修正，請輸入N:\n"
                )
                if temp == "N":
    ...
    if len(require) == 0:
        require.append("N")
    general = input(
        "請輸入通識的向度或領域(如果同一個領域要修兩堂或是有不同領域則以空格替代):\n"
        "向度直接填數字，領域則填(自、社、人)"
        "ex:修兩堂向度1的課輸入 1 1\n如果沒有則輸入N\n"
    ).split()
    if general == "N":
    ...
    my_condition = condition(
        ...
        GPA=GPA,
        grade=grade,
        credit_upperbound=int(credit[1]),
        credit_lowerbound=int(credit[0]),
        require=require,
        general=general,
    )
    return my_condition
    ...
    if require[0] != "N":
        ...
        for i in require:
            ...
            if required_lesson["Time"][0] == "ALL":
                ...
                curriculum["Else"][index[index_count]] = required_lesson[
                    "Course Title"
                ][0]
            else:
                total_GPA += required_lesson["Average GPA"][0]
...
class GA:
    def __init__(
        ...
        if (not isinstance(k, int)) or (k < 1) or (k > pop_size):
            raise ValueError(
                "`k` for tournament selection can only an integer between 1 and `pop_size`."
            )
            ...
            if index == 1:
                ...
                for j, char in enumerate(str(class_period)):
                    if char in ["M", "T", "W", "R", "F"]:
                        ...
                        if i < len(self.major):
                            curr_copy.iloc[time_index - 1, day_index] = self.major.iloc[
                                i, 1
                            ]
                        else:
                            curr_copy.iloc[time_index - 1, day_index] = self.GE.iloc[
                                i - len(self.major), 1
                            ]
        if class_num != 0:
            averange_GPA = (total_GPA + self.base_total_GPA) / (
                class_num + self.base_total_class
            )
        else:
            averange_GPA = 0
...
# combine all lessons
dataset = pd.concat([PME_dataset, ESS_dataset, GE_dataset], join="outer").reset_index(
    drop=True
)
# user input
my_condition = interface(list(dataset["Course No."]))
curriculum_pre, total_class, total_GPA, total_credit = lesson_required(
    my_condition.require, dataset
)
...
if my_condition.department == "ESS":
    ESS_selected = ESS_dataset[
        ESS_dataset["Grade"] == str(my_condition.grade)
    ...
    PME_selected = PME_dataset[
        PME_dataset["Grade"] == str(my_condition.grade)
    ].reset_index(drop=True)
    curriculum_final = GA(
        condition=my_condition,
...
for i in range(GA_GENERATION):
    ...
    print("\n")
    print(
        f"Generation {(i + 1): 4d}, best fitness = {curriculum_final.best_so_far.fitness:4.2f}"
    )
    curriculum_final.evaluate(curriculum_final.best_so_far.gene)
    curriculum_final.best_so_far.print()
...
curriculum_final.best_so_far.curr.to_csv(
    "curriculum.csv", index=True, header=True, encoding="big5"
)
